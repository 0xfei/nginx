...
/*
    copy src from pool, of cource should alloc memory
*/
u_char *
...
/*
    real printf
    l mean last address, based on buf
    fmt mean format 
    args is args
*/
u_char *
ngx_vslprintf(u_char *buf, u_char *last, const char *fmt, va_list args)
{
    ...
    while (*fmt && buf < last) {
        ...
        if (*fmt == '%') {
            ...
            // second %. control and write args to buf
            switch (*fmt) {
            ...
            case 'V':
                ...
                // ngx_cpymem memcpy and change buf to buf+n
            ...
            case 'c':
                // intresting !!! %c is treat with char but the param itself is a int
                d = va_arg(args, int);
                *buf++ = (u_char) (d & 0xff);
                fmt++;
...
/*
    output number with \width\sign\hex\padding\
    intresting thing is we deal the number backward!!!
*/
static u_char *
    ...
    ngx_uint_t hexadecimal, ngx_uint_t width)
{
    ...
    if (hexadecimal == 0) {
        ...
        if (ui64 <= (uint64_t) NGX_MAX_UINT32_VALUE) {
            ...
            do {
                // intresting !!! backward write the number!!!!!
                *--p = (u_char) (ui32 % 10 + '0');
            } while (ui32 /= 10);
...
/*
    html escape 
    two step: 1. dst==NULL get length 2. dst=length get escape
*/
uintptr_t
...
/*
    json escape, \\ or ascii or \\u000(space)
    escape and move or return the length
*/
uintptr_t
...
/*
    rbtree insert value, just a function param of tree_node_t
    balance mantain called after this insert
*/
void
    ...
    ngx_rbtree_node_t *node, ngx_rbtree_node_t *sentinel)
{
    ...
    // remember, there is only one sentinel!!! so a root and a real son!!
    *p = node;
    node->parent = temp;
    node->left = sentinel;
    node->right = sentinel;
